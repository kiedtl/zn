...
_PUSH(& inputstr , ( /*typ = array_string   tmp_typ=string*/ line ), tmp28, string) ;
...
_PUSH_MANY(& res , ( /*typ = array_string   tmp_typ=string*/ os__walk_ext ( p , ext ) ), tmp36, array_string) ;
...
os__Result tmp6 = OPTION_CAST(os__Result)( (os__Result) { .output =  res , .exit_code =  exit_code }); 
 return opt_ok(&tmp6, sizeof(os__Result)) ;
 
 }
 void stdlib__die(string reason) {
 
printf( "ERROR: %.*s\n", reason.len, reason.str ) ;
 
 v_exit ( 1 ) ;
 
 }
 void stdlib__defptr(array_byte* memory, map_array_int* variables, array_string command) {
 
string ptr_name= ( *(string*) array_get(  command , 1) ) ;
 
string start_addr= ( *(string*) array_get(  command , 2) ) ;
 
string end_addr= ( *(string*) array_get(  command , 3) ) ;
 
 if ( string_starts_with ( ptr_name , tos3("%") )  ||  string_starts_with ( ptr_name , tos3("&") )  ||  string_starts_with ( ptr_name , tos3("*") ) ) {
 
 stdlib__die ( tos3("ptr names may not start with `%`, `&`, or `*`!") ) ;
 
 }
 ;
 
 if ( ! string_starts_with ( start_addr , tos3("%") )  ||  ! string_ends_with ( end_addr , tos3("%") ) ) {
 
 stdlib__die ( _STR("invalid addresses: %.*s and/or %.*s", start_addr.len, start_addr.str, end_addr.len, end_addr.str) ) ;
 
 }
 ;
 
map_set(& variables [/*ptr*/ , ptr_name ]/*rarray_int 1*/ , & (array_int []) { new_array_from_c_array(2, 2, sizeof(int), EMPTY_ARRAY_OF_ELEMS( int, 2 ) {  v_string_int ( string_replace ( start_addr , tos3("%") , tos3("") ) ) ,  v_string_int ( string_replace ( end_addr , tos3("%") , tos3("") ) )  }) }) ;
...
array_byte memory=new_array_from_c_array(0, 0, sizeof(byte), EMPTY_ARRAY_OF_ELEMS( byte, 0 ) {   TCCSKIP(0) }) ;
 
map_array_int pointers= new_map(1, sizeof(array_int)) ;
 
array_string files= os__args ;
 
 array_string tmp1 =  files;
 for (int tmp2 = 0; tmp2 < tmp1.len; tmp2++) {
 ...
 thread_arg_main__read * tmp3 = malloc(sizeof(thread_arg_main__read));
 tmp3 -> file =  
 file ;
 tmp3 -> memory =  
 memory ;
 ...
 pointers ;
 pthread_t _thread4;
 int tmp5 = pthread_create(& _thread4, NULL, (void *)main__read_thread_wrapper,  tmp3);
 ;
 ...
 void main__read(string file, array_byte* memory, map_array_int* variables) {
